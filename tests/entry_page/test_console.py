...
# import pathlib
# import time
from bd_crossword.entry_page import console
...
# mock_requests_get, mock_time_sleep
def test_main_succeeds(runner):
    result = runner.invoke(console.main, ["--page-date-string", "2022-01-11"])
    logger.debug("vars : %s", vars(result))
    print(f"======================== {str(type(result))}")
    logger.debug("type : %s", str(type(result)))
    logger.debug("output : %s", result.output)
    print(result.exit_code)
    logger.debug("exit_code : %s", str(result.exit_code))
...
#     assert setup_dump_files["html_dump_file"].read_text().
